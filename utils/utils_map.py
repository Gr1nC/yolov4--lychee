...
"""
 Convert the lines of a file to a list，计算ground_true
"""
...
"""
 不知道为什么两个文件的编码格式不一样，所以我重新写了个函数完成编码,计算detection_results
"""
#
...
def get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path = './map_out'):
    ...
    for txt_file in ground_truth_files_list:
        ...
        if not os.path.exists(temp_path):
            ...
            error(error_msg)
        lines_list      = detection_file_lines_to_list(txt_file)
        bounding_boxes  = []
        is_difficult    = False
        ...
        with open(TEMP_FILES_PATH + "/" + file_id + "_ground_truth.json", 'w', errors='ignore',encoding='utf-8') as outfile:
    ...
    for class_index, class_name in enumerate(gt_classes):
        ...
        for txt_file in dr_files_list:
            ...
            if class_index == 0:
                if not os.path.exists(temp_path):
                    error_msg = "Error. File not found: {}\n".format(temp_path)
                    error(error_msg)
            lines = detection_file_lines_to_list(txt_file)
        ...
        with open(TEMP_FILES_PATH + "/" + class_name + "_dr.json", 'w', errors='ignore',encoding='utf-8') as outfile:
            json.dump(bounding_boxes, outfile)

    sum_AP = 0.0
    ap_dictionary = {}
    lamr_dictionary = {}
    with open(RESULTS_FILES_PATH + "/results.txt", 'w', errors='ignore',encoding='utf-8') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            dr_data = json.load(open(dr_file, errors='ignore',encoding='utf-8'))
            ...
            for idx, detection in enumerate(dr_data):
                ...
                ground_truth_data   = json.load(open(gt_file, errors='ignore',encoding='utf-8'))
                ovmax       = -1
                gt_match    = -1
                ...
                if ovmax >= min_overlap:
                    if "difficult" not in gt_match:
                        if not bool(gt_match["used"]):
                            ...
                            with open(gt_file, 'w', errors='ignore',encoding='utf-8') as f:
    ...
    """
    with open(RESULTS_FILES_PATH + "/results.txt", 'a', errors='ignore',encoding='utf-8') as results_file:
...
def get_coco_map(class_names, path):
    ...
    with open(GT_JSON_PATH, "w", errors='ignore',encoding='utf-8') as f:
        results_gt  = preprocess_gt(GT_PATH, class_names)
        json.dump(results_gt, f, indent=4)

    with open(DR_JSON_PATH, "w", errors='ignore',encoding='utf-8') as f:
